    ...
    (
        ...
        function(EventEmitter2) {
          var MjpegCanvas = function(options) {
            ...
            if (mjpegCanvas.topic instanceof Array) {  //判断topic是否是数组
              // check the labels
              if (!(mjpegCanvas.label && mjpegCanvas.label instanceof Array && mjpegCanvas.label.length == mjpegCanvas.topic.length)) {  //label是否为空+label个数=topic数+长度相等
                mjpegCanvas.label = null;
              }

              // start the stream with the first topic
              curStream = mjpegCanvas.topic[mjpegCanvas.defaultStream]; //默认[0]
