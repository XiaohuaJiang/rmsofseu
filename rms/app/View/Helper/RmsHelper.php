 ...
 */
	public function ros2d($background = '#111111', $heightScale = 0.66) {
		...
		$html .= __(
			'{divID:"viewer2d",width:w*7/8,height:w*%f,antialias:true,background:"%s"});', $heightScale, $background
		);
		$html .= '</script>';
 ...
 */
	public function ros3d($background = '#111111', $intensity = 0.66, $heightScale = 0.66) {
		...
		$html .= __(
			'{divID:"viewer",width:w*7/8,height:w*%f,antialias:true,background:"%s",intensity:%f});',
			$heightScale,
			$background,
			$intensity
		);
 ...
 * @return string The HTML for the entire script block.
 */
	public function ros3dmap($background = '#111111', $intensity = 0.66, $heightScale = 1) {
		$html = '<div id="map"></div>';
		$html .= '<script>';
		$html .= 'var w=Math.min($("#map").parent().width(), 5000000);';
		//$html .= 'var h=486;';
		// create the viewer
		$html .= '_VIEWER = new ROS3D.Viewer(';
		$html .= __(
			'{divID:"map",width:w/3,height:w*%f/3,antialias:true,background:"%s",intensity:%f});',
			$heightScale,
			$background,
			$intensity
		);
 ...
 * @return string The HTML for the entire script block.
 */
	public function laserscan($topic) {
		$html = '<script>';
		// create the ROS connection
		$html .= 'new ROS3D.MarkerClient({ros:_ROS,tfClient:_TF,rootObject:_VIEWER.scene,';
		$html .= __('topic:"%s"', h($topic));
 ...
 * @return string The HTML for the entire script block.
 */
	public function gridcell($topic) {
		$html = '<script>';
		// create the ROS connection
		$html .= 'new ROS3D.MarkerClient({ros:_ROS,tfClient:_TF,rootObject:_VIEWER.scene,';
		$html .= __('topic:"%s"', h($topic));
 ...
 * @return string The HTML for the entire script block.
 */
	public function posearray($topic) {
		$html = '<script>';
		// create the ROS connection
		$html .= 'new ROS3D.MarkerClient({ros:_ROS,tfClient:_TF,rootObject:_VIEWER.scene,';
		$html .= __('topic:"%s"', h($topic));
...
/**
 * Create a keyboard batteryinfo connection via keyboardbatteryinfojs.
 *
 * @param string $topic The teleoperation topic to publish to.
 * @param null|string $throttle The throttle rate.
 * @return string The HTML for the entire script block.
 */
	public function keyboardBatteryinfo($topic, $throttle = null) {
		$html = '<script>';
		// create the telop connection
		$html .= __('_BATTERYINFO = new KEYBOARDBATTERYINFO.Batteryinfo({ros:_ROS,topic:"%s"});', h($topic));
		if ($throttle) {
			$html .= __('_BATTERYINFO.throttle=%f', h($throttle));
		} else {
			$html .= __(
				'_BATTERYINFO = new KEYBOARDBATTERYINFO.Batteryinfo({ros:_ROS,topic:"%s",throttle:%f});', h($topic), h($throttle)
			);
		}
		$html .= '</script>';
