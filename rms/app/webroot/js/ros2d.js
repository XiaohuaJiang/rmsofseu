...
ROS2D.OccupancyGrid = function(options) {
  ...
  canvas.width = this.width;		//地图宽度
  canvas.height = this.height;		//地图长度
...
ROS2D.OccupancyGridClient = function(options) {
  ...
  this.rootObject = options.rootObject || new createjs.Container();	//600 * 400
  ...
  rosTopic.subscribe(function(message) {
    ...
    that.currentGrid = new ROS2D.OccupancyGrid({	//create a map base on message
      message : message
    });
    that.rootObject.addChild(that.currentGrid);		//append the map on the stage
...
/**
 * A rectangle that can be used to display camera.
 *
 * @constructor
 * @param options - object with following keys:
 *   * size (optional) - the size of the marker
 ...
 *   * fillColor (optional) - the createjs color for the fill
 */
 ROS2D.DisplayRectangle = function(options) {
  var that = this;
  options = options || {};
...
ROS2D.DisplayRectangle.prototype.__proto__ = createjs.Shape.prototype;
 ...
 * A line that can be used to display plan.
 *
 * @constructor
 * @param options - object with following keys:
 ...
 */
 ROS2D.DisplayPlan = function(options) {
  var that = this;
  options = options || {};
  var size = options.size || 10;
  var strokeSize = options.strokeSize || 3;
  var strokeColor = options.strokeColor || createjs.Graphics.getRGB(255, 128, 0,0.66);
  var fillColor = options.fillColor || createjs.Graphics.getRGB(255, 255, 255);
  var plan = options.plan || 0;
  var len=plan.cells.length;
  var width = options.width || width0;
  var height = options.height || height0;
    //confirm("Are you sure? len="+len);
  // draw the rectangle
  var graphics = new createjs.Graphics();
  // line width
  graphics.setStrokeStyle(strokeSize);
  graphics.moveTo((plan.cells[0].x-plan.cells[0].x)*size*1.0/(width*1.0), -(plan.cells[0].y-plan.cells[0].y)*size/(height*1.0));
  graphics.beginStroke(strokeColor);
  //graphics.beginFill(fillColor);
  for(var i=1;i<len;i++)
  {graphics.lineTo((plan.cells[i].x-plan.cells[0].x)*size/(width*1.0), -(plan.cells[i].y-plan.cells[0].y)*size/(height*1.0));}
 ...
 /* var graphics = new createjs.Graphics();
  // line width
  graphics.setStrokeStyle(strokeSize);
  graphics.moveTo(-size / 8.0, size / 8.0);
  graphics.beginStroke(strokeColor);
  graphics.beginFill(fillColor);
  graphics.lineTo(-size / 8.0,-size / 8.0);
  graphics.lineTo(size / 8.0,-size / 8.0);
  graphics.lineTo(size / 8.0,size / 8.0);
  graphics.lineTo(-size / 8.0,size /8.0);
  graphics.closePath();
...
ROS2D.DisplayPlan.prototype.__proto__ = createjs.Shape.prototype;
/**
 * @author Russell Toris - rctoris@wpi.edu
 */
